...
def extract_moyen(ecole_url):
    ...
    if condition():
        ...
        classes = Classes.objects.filter(level__gt=0).values_list('id', flat=True)
        eleves_dic = {}
        ...
        def eleves_racha_age():
            ...
            for i in range(1, 7):
                classes_ofLevel = Classes.objects.filter(
                    level=i).values_list('id', flat=True)
            ...
            for eleve in tr_s:
                ...
                if eleve_id != '':
                    ...
                    eleve = Eleves.objects.annotate(full_name_concat=Concat(F('nom'), Value('  '), F('prenom'))
                                                    ).filter(full_name_concat__iexact=nom_prenom, id=None)
                    if eleve:
                        is_graduated = None
                        resultat = None
                        if moyen_float >= 10:
...
def ta7dhir_is_graduated():
    classes_with_level_0 = Classes.objects.filter(level='0')
    alleleves_t7dhiri = []
    for class_id in classes_with_level_0:
        eleves = Eleves.objects.filter(class_id=class_id.id)
        for eleve in eleves:
...
def initiate(dic):
    ...
    # Eleves_arrives(dic['ecole_url'])  kamlha idk u stopped nxt day u forgot
...
def chgmentClas1(ecole_url):
    ...
    for classe in sorted_classes:
        ...
        for eleve in elevsClass:
            inceremnt += 1
            eid = eleve.eid
            if (eleve.next_class_id == None or eleve.next_class_id == eleve.class_id):
                nxt_class = "rien"
            else:
                nxt_class = eleve.next_class_id.id
